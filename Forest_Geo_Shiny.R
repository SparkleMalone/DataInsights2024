...
forest_geo <- read_excel("data/ForestGEO_Data_Final_2019.xlsx") %>% as.data.frame()


forest_geo <- forest_geo %>% 
              mutate(DBH = as.numeric(DBH)) %>%
              filter(DBH<1000) %>%
              mutate(area = (DBH/2)^2*pi)

hist(as.numeric(forest_geo$DBH[forest_geo$DBH<1000]))
hist(as.numeric(forest_geo$DBH[forest_geo$DBH<5]))

sum(forest_geo$area)/50

ui <- fluidPage(
  titlePanel("DBH error estimation"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output) {
  ...
  output$distPlot <- renderPlot({
    dbh_data <- forest_geo$DBH
    biltmore_data <- forest_geo$DBH + rnorm(length(forest_geo$DBH), mean = 0, sd = input$biltmore_error)
